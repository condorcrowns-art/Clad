"""Builds structurally valid, non-executable PE files.

Used by the test suite to exercise the PE parser without Windows, and by the
coverage self-test to create benign sample files that are recognisably PE but
contain no code that could ever run. Every header field the parser reads is
genuine; the "code" section is filler.
"""
...
def build_test_pe(*, original_filename: str | None = None, payload: bytes = b"") -> bytes:
    """A benign PE for self-tests, optionally declaring an original file name."""
    strings = {"OriginalFilename": original_filename} if original_filename else None
    body = payload or b"CandySelfTest benign sample, contains no executable code. " * 8
    return build_pe(strings=strings, code=body)
